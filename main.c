...
int read_data_from_console(char* card_title, char* bank_name) {
    ...
    int scan_res = scanf("%30s", card_title);
    if (scan_res != 1) {
        return 1;
    }
...
struct bank_t* get_bank(struct bank_t* banks, int* number_of_banks, char* bank_name) {
    if (number_of_banks == NULL || bank_name == NULL) {
        return NULL;
    }
    if (banks == NULL && *number_of_banks > 0) {
        return NULL;
...
//        }
    ...
    (banks + numb_of_banks)->name = malloc(31);
...
//    if (number_of_cards == NULL || card_title == NULL || bank == NULL) {
//        return NULL;
//    }
//    if (cards == NULL && *number_of_cards > 0) {
//        return NULL;
//    }
    ...
    if (numb_of_cards != 0) {
        printf("%d\n", sizeof(struct card_t) * (numb_of_cards+1));
...
int main() {
    ...
    for (int i=2; i < 10; i++) {
        printf("%d\n", sizeof(struct bank_t) * (i + 1));
        struct bank_t* new_banks = (struct bank_t*)realloc(banks, sizeof(struct bank_t) * (i + 1));
        printf("good!\n");
        if (new_banks == NULL) {
            printf("Failed to allocate memory");
            return 1;
...
//        struct card_t* card = add_card(cards, &number_of_cards, card_title, NULL);
//        if (card == NULL) {
//            free(card_title);
//            free(bank_name);
//            destroy_banks(banks, number_of_banks);
